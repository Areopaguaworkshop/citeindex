...
pub fn execute(
    ...
    let reader = ctx.document_index.reader()
    ...
    if let Ok(f) = schema.get_field(&title_field) { search_fields.push(f); }
    if let Ok(f) = schema.get_field(&abstract_field) { search_fields.push(f); }
    if let Ok(f) = schema.get_field("authors_en") { search_fields.push(f); }
    if let Ok(f) = schema.get_field("keywords") { search_fields.push(f); }
    ...
    for (raw_score, doc_address) in &top_docs {
        ...
        for fv in doc.field_values() {
            ...
            match name {
                "doc_id" | "title" | "authors" | "doi" | "abstract_text"
                | "venue" | "doc_type" | "quality_tier" | "language"
                | "merkle_hash" => {
