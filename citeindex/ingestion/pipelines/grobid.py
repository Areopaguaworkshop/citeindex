...
def extract_document_metadata_grobid(
    ...
    """Extract the document's own metadata via GROBID's header endpoint.

    Sends the PDF to ``/api/processHeaderDocument`` and returns a
    CSL-compatible dict with title, authors, abstract, and date.
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.warning(
                "GROBID header request failed with status %d", resp.status_code
            )
            return {}
        ...
        try:
            ...
            logger.warning("Failed to parse TEI XML from header response")
            return {}
