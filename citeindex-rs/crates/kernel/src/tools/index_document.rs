...
pub fn execute(
    ...
    let doc_type = params.get("doc_type").and_then(|v| v.as_str()).unwrap_or("article-journal");
    ...
    if let Ok(f) = schema.get_field("year") { tantivy_doc.add_i64(f, year); }
    ...
    let mut writer = ctx.document_writer.lock()
